...
class QualificationAgent:
    ...
    def _question_for_field(self, field: str) -> str:
        if field == "location":
            invalid_attempt = self.memory.data.get("invalid_location_attempt")
            if invalid_attempt:
                self.memory.data["invalid_location_attempt"] = ""
                self.memory.save()
                return f"We don't have a branch in {invalid_attempt}. Our active locations are Koramangala, Whitefield, and JP Nagar. Which of these works best for you?"
    ...
    def _capture_expected_field(self, message: str, expected: str, intent: str) -> None:
        ...
        if expected == "preferred_date":
            ...
            if location:
                ...
                invalid_locs = (
                    "malleswaram", "indiranagar", "jayanagar", "hsr", "marathahalli",
                    "brookefield", "electronic city", "ecity", "btm", "banashankari",
                    "sadashivanagar", "mg road", "ulsoor", "bellandur"
                )
                matched_invalid = next((loc for loc in invalid_locs if loc in lowered), "")
                if matched_invalid:
                    self.memory.data["invalid_location_attempt"] = matched_invalid.title()
                    self.memory.save()
