...
class BookingOrchestrator:
    ...
    def _check_availability_operational(self, location: str, date: str, participants: int, room: str = "") -> dict[str, Any]:
        ...
        try:
            ...
            alternative_games = []
            if not bookable_display_slots:
                try:
                    all_games = self.booking_provider.get_booking_games(location_id) or []
                    for g in all_games:
                        g_min = self._optional_int(g.get("peopleMin"))
                        g_max = self._optional_int(g.get("peopleMax"))
                        g_name = self._record_value(g, ("gameName", "name", "title"))
                        if g_name and g_name.lower() != room.lower():
                            if (g_min is None or participants >= g_min) and (g_max is None or participants <= g_max):
                                alternative_games.append(g_name)
                except Exception:
                    pass
