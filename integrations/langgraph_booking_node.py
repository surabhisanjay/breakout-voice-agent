...
# code remains importable even when `langgraph` isn't installed in the
# developer environment.

# Note: Replace the pseudo-API calls with the actual LangGraph SDK calls
# when wiring into an environment that provides `langgraph`.
# """
...
#     """Handle a handoff payload and attempt to create a booking.

#     Expects `handoff` to contain keys like `intent`, `location`, `preferred_date`,
#     `participants`, `customer_name`, and `phone` (the structure produced by `HandoffGenerator`).
#     Returns a dict with booking status and payload suitable for downstream nodes.
#     """
#     agent = BookingAgent()

#     # For demo purposes: create a matching slot if none exists.
#     service = handoff.get("intent", "escape_room_inquiry")
#     date = handoff.get("preferred_date") or "2026-07-01"
#     # Pick a time slot (production: query calendar/inventory)
...
from src.booking_agent import BookingSimulator as BookingAgent, BookingError, EscalationRequired
...
def run_booking_workflow(handoff: Dict[str, Any], require_payment: bool = False, agent: Optional[BookingAgent] = None) -> Dict[str, Any]:
