...
class EscalationAgent:
    ...
    COMPLAINT = re.compile(
        r"\b(?:bad\s+experience|terrible|horrible|did\s+not\s+like|didn't\s+like|not\s+happy|disappointed|unsatisfied|unacceptable|poor\s+service|last\s+time\s+was|waste\s+of\s+money|waste\s+of\s+time|worst|this\s+is\s+ridiculous|you(?:'re|\s+are)\s+not\s+helping|called\s+multiple\s+times)\b",
        re.IGNORECASE,
    )
    ...
    def evaluate(
        ...
        recent_customer_turns = [
            re.sub(r"[^a-z0-9 ]", "", re.sub(r"\s+", " ", str(turn.get("content", "")).lower())).strip()
            for turn in self.memory.data.get("conversation", [])
            if turn.get("role") == "customer" and turn.get("content")
        ][-3:]
        repeated_question = bool(
            len(recent_customer_turns) >= 3
            and len(set(recent_customer_turns[-3:])) == 1
            and not self._is_active_booking_update(message)
        )
        ...
        elif self.COMPLAINT.search(message):
        ...
        elif repeated_question or repeated_loop:
        ...
        category, priority, recommended_action = self._classification(reason)
        ...
        transfer_required = bool(reason)
        transfer_status = "not_required"
        if transfer_required:
            transfer_status = "requested" if os.environ.get("HUMAN_TRANSFER_DESTINATION") else "pending_configuration"
    ...
    @staticmethod
    def _classification(reason: str) -> tuple[str, str, str]:
        mapping = {
            "Customer reported an immediate safety concern": ("safety", "critical", "Immediately route to human support and stop automated handling."),
            "Customer explicitly requested a human representative": ("human_request", "high", "Transfer the active call to an available human agent."),
            "Payment issue requires manual support": ("payment", "high", "Verify payment status, preserve the booking context, and reconcile the transaction."),
            "Multiple failed authentication attempts": ("authentication", "high", "Stop sensitive operations and complete manual identity verification."),
            "Customer requested a refund": ("refund", "high", "Review the booking and payment records before any refund action."),
            "Customer anger detected": ("sentiment", "high", "Transfer to a human agent with priority handling."),
            "Repeated customer frustration detected": ("sentiment", "high", "Transfer to a human agent with the conversation summary."),
            "Repeated misunderstanding reported by customer": ("conversation_failure", "high", "Review the unresolved request and continue with a human agent."),
            "Repeated conversation loop detected": ("conversation_failure", "high", "Review the repeated question and continue with a human agent."),
            "Agent could not answer the customer query": ("knowledge_gap", "normal", "Answer the saved unresolved query and update the knowledge base."),
            "Booking or integration failure": ("booking_failure", "high", "Check booking API health and complete the booking manually if needed."),
            "Multiple agent or integration failures": ("integration_failure", "high", "Inspect integration errors and continue the request manually."),
            "Policy or refund dispute": ("refund", "high", "Review policy applicability and booking evidence with the customer."),
        }
        return mapping.get(reason, ("general", "normal", "Review the conversation and contact the customer."))
