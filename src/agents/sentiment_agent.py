...
class SentimentAgent:
    ...
    def _rule_based(self, message: str) -> tuple[str, float, str]:
        if re.search(r"\b(?:no\s+no\s+no|that's not what i asked|that is not what i asked|you are not understanding|not understanding|you don't understand|you are not listening|my booking is wrong|booking is wrong|i already told)\b", message):
            return "frustrated", 0.94, "Customer reported repeated misunderstanding."
